...
import com.cabletech.bj.services.WapWaitHandleWorkBO;
import com.cabletech.commons.generatorID.impl.OracleIDImpl;
import com.cabletech.commons.hb.QueryUtil;
import com.cabletech.commons.hb.UpdateUtil;
import com.cabletech.commons.md5.MD5;
import com.cabletech.commons.util.CollectionUtil;
 ...
 * 
 */
public class WapLoginAction extends BaseInfoBaseDispatchAction {
	private Logger logger = Logger.getLogger(WapLoginAction.class);
	...
	public ActionForward login(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		if (!logindao.validateUserExist(userID)) {
			...
			request.getSession().setAttribute("FIRSTMENUMAP", CollectionUtil.array2Map(firstMenu));
			...
			WapWaitHandleWorkBO waitHandleWorkBo = (WapWaitHandleWorkBO) ctx.getBean("wapWaitHandleWorkBO");
			// 验证用户是否登陆
			waitHandleWorkBo.processMenuVector(firstMenu, userInfo);
			request.setAttribute("first_menu", firstMenu);

			WapOnLineBO onlineBo = (WapOnLineBO) ctx.getBean("wapOnLineBO");
			String mobileHTML = onlineBo.getOnlinePatrolman(userInfo);
