...
<html>
	<head>
		...
		toViewWorkStatisticForm = function (dispatchId, inputUrl) {
			var url = contextPath + "/sendTaskWorkStatisticAction_oneWorkStatistic.jspx?id=" + dispatchId + "&&backUrl=" + inputUrl;
			location.href = url;
		};
		</script>
	</head>
	<body>
		...
		<link rel="stylesheet" href="${ctx}/css/screen.css" type="text/css"
			media="screen, print" />
		<br>
		<template:titile value="工作量统计" />
		<s:form action="sendTaskWorkStatisticAction_workStatistic"
			method="post">
		...
		<c:if test="${sessionScope.SEND_TASK_LIST!=null}">
			<display:table name="sessionScope.SEND_TASK_LIST"
				requestURI="${ctx}/sendTaskWorkStatisticAction_workStatistic.jspx"
				...
				<fmt:formatNumber pattern="#0.000"
					value="${currentRowObject.worknum}" var="workNum"></fmt:formatNumber>
				<display:column media="html" title="主题" sortable="true"
					maxLength="40">
					<a
						href="javascript:toViewForm('${dispatchId }','/sendTaskWorkStatisticAction_workStatistic.jspx')">
						${sendTopic }</a>
				</display:column>
				...
				<display:column property="sendtime" title="派发时间" style="width:140px"
					sortable="true" />
				<display:column title="目前工作量（单位：米）" style="width:180px"
					sortable="true">
					${workNum }
				</display:column>
				<display:column property="workunit" title="施工单位" style="width:140px"
					sortable="true" />
				<display:column media="html" title="操作" style="width:100px">
					<a
						href="javascript:toViewWorkStatisticForm('${dispatchId }','/sendTaskWorkStatisticAction_workStatistic.jspx');">查看工程进度</a>
					<a href="#">连接GIS</a>
				</display:column>
			</display:table>
			<a href="#">导出为Excel文件</a>
