...
function businessTypeChange(obj){
		...
		<title>复制巡检计划信息</title>
	</head>
	<body>
		<template:titile value="复制计划模板信息" />
		<s:form action="/patrolManager/wplanTemplateAction_copy.jspx" id="wplantemplate" name="wplantemplate" method="post">
		    <input type="hidden" id="vo.businessType" name="vo.businessType" value="${vo.businessType }"/>
			<template:formTable >
				...
				<template:formTr name="备注">
					<textarea rows="3" name="vo.remark"  style="width: 220px;">${vo.remark }</textarea>
				</template:formTr>
				...
				<template:formSubmit>
					<td>
						<input type="button" onclick="valiSub()" id="button" class="button" value="创建">
					</td>
					<td>
					</td>
					<td>
