...
<script type="text/javascript"
	src="${ctx }/bs/fault/js/fault_dispatch_waithandled_list.js"></script>
...
<template:titile value="待办故障派单信息列表" />
<s:form action="faultDispatchAction_waitHandledList.jspx" method="post">
...
<display:table name="sessionScope.FAULT_DISPATCH_LIST" id="row"
	...
	<display:column media="html" title="操作" paramId="id">
		<!-- 
		<a href="javascript:reply('${row.dispatch_id}','${row.task_id }')">回单</a>
		<a href="javascript:audit('${row.dispatch_id}','${row.task_id }')">审核</a>
		 -->
		<a
			href="javascript:viewFaultDispatch('${row.dispatch_id}','${row.business_type }')">查看</a>
		<a
			href="javascript:toDoTaskForm('${row.url}','${row.business_type }')">办理</a>
	</display:column>
</display:table>
