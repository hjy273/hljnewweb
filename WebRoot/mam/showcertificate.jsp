...
<html>
	<head>
        ...
        function toGetForm(idValue){
            var url = "${ctx}/CertificateAction.do?method=showOneCertificate&id=" + idValue;
        ...
        function toUpForm(idValue){
            var url =  "${ctx}/CertificateAction.do?method=showUp&id=" + idValue;
            self.location.replace(url);
        }

        function toDeletForm(idValue){
        	if(confirm("你确定要执行此次删除操作吗?")){
            	var url =  "${ctx}/CertificateAction.do?method=deleteInfo&id=" + idValue;
            	self.location.replace(url);
        	}
        	else
        		return ;
	...
	<body>
		...
		<template:titile value="代维资质信息一览表" />
