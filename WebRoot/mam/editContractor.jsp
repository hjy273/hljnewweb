...
function isValidForm() {
	...
	return true;
    	...
    	judgeExist=function(){
			...
			var queryString="contractorName="+contractorName+"&contractorID=<bean:write name='contractorBean' property='contractorID' />&rnd="+Math.random();
...
<html:form onsubmit="return isValidForm()" method="Post"
	...
	<table width="720" border="0" align="center" cellpadding="3"
		...
		<tr class="trwhite">
			...
			<td class="tdulright" style="width: 35%">
				<html:text property="alias" styleClass="inputtext" style="width:160" />
			</td>
		</tr>
		<tr class="trwhite">
			<td class="tdulleft" style="width: 15%">
		...
		<tr class="trwhite">
			...
			<td class="tdulright" style="width: 35%">
				<select name="organizationType" class="inputtext" style="width: 160"
					onchange="showCompanyInfoTb(this.value);">
					<option value="1">
						独立法人
					</option>
					<option value="2">
				...
				document.contractorBean.organizationType.value="<bean:write name='contractorBean' property='organizationType' />";
				</script>
			</td>
			<td class="tdulleft" style="width: 15%">
				所属区域：
		...
		<tr class="trwhite">
			...
			<td class="tdulright" style="width: 85%" colspan="3">
				<c:forEach var="res" items="${resources}">
					<c:set var="ch" value=""></c:set>
					<c:forEach var="rsid" items="${contractorBean.resourcesId}">
						<c:if test="${rsid==res.code}">
							<c:set var="ch" value="checked"></c:set>
						</c:if>
					</c:forEach>
					<input type="checkbox" name="resourcesId" value="${res.code}" ${ch}/>
			...
			<tr class="trwhite">
				...
				<td class="tdulright" style="width: 85%" colspan="3">
					<textarea cols="5" rows="2" name="remark" style="width: 280;"
						class="textarea"><bean:write name='contractorBean' property='remark' /></textarea>
					<font color="#FF0000">*</font>
				</td>
			</tr>
		</tbody>
		...
		<tr class="trwhite">
			...
			<td class="tdulright" style="width: 85%" colspan="3">
				<html:text property="email" styleClass="inputtext" style="width:160" />
			</td>
		</tr>
		<template:formSubmit>
			<td>
...
<script type="text/javascript">
showCompanyInfoTb("<bean:write name='contractorBean' property='organizationType' />");
</script>
