...
<body>
<template:titile value="查询用户操作日志"/>
<html:form  styleId="form1"   method="Post" action="/BusinesslogAction.do?method=queryLogList">
  ...
  <%}  %>
        ...
        <logic:present name="uinfo">
          <logic:iterate id="uinfoId" name="uinfo">
            <option value="<bean:write name="uinfoId" property="userid"/>">
              <bean:write name="uinfoId" property="username"/> -- <bean:write name="uinfoId" property="userid"/>
            </option>
          </logic:iterate>
        </logic:present>
      </select>
    ...
    <template:formTr name="开始日期">
      <input type="text" name="beginDate" class="Wdate inputtext" size="25" maxlength="45" readonly="readonly" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd'})"/>
    </template:formTr>
    <template:formTr name="结束日期">
      <input type="text" name="endDate" class="Wdate inputtext" size="25" maxlength="45" readonly="readonly" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd'})"/>
    </template:formTr>
    <template:formSubmit>
      <td>
