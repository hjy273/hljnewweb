...
<body >
	...
	<div class="bg">
		...
		<div class="bg_right">
			<div class="${msginfo.errorlevel}"></div>
			<div class="text">${msginfo.message}</div>
			<div class="layout_button">
			<c:if test="${'返回'==msginfo.link}">
				 <input name="Submit" type="submit" class="button" value="" style="cursor:hand" onClick="javascript:history.go(-1);">
			</c:if>
			<c:if test="${'返回'!=msginfo.link}">
				 <input name="Submit" type="submit" class="button" value=" " style="cursor:hand"
				 onClick="javascript:location.href='${msginfo.link}'">
			</c:if>
			</div>
		</div>
	</div>
