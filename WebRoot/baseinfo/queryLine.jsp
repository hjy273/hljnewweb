...
<html:form method="Post" action="/lineAction.do?method=queryLine">
  <template:formTable contentwidth="200" namewidth="200">
    <template:formTr name="线路编号" >
      <html:text property="lineID" styleClass="inputtext" style="width:160" maxlength="45"/>
    </template:formTr>
    <template:formTr name="线路名称" >
      <html:text property="lineName" styleClass="inputtext" style="width:160" maxlength="45"/>
    </template:formTr>

    <logic:notEmpty name="reginfo">
          <template:formTr name="所属区域" >
            <select name="regionid" class="selecttext" style="width:160px" id="rId"  onclick="onChangeDept()" >
    ...
    <logic:notEmpty name="deptinfo">
          <template:formTr name="所属部门" tagID="deptTr" >
            <select name="ruleDeptID" class="selecttext" style="width:160px" id="dId">
              <option value="">不限</option>
          ...
          <logic:present name="deptinfo">
              <select name="workID" class="selecttext">
                <logic:iterate id="deptinfoId" name="deptinfo">
    ...
    <apptag:setSelectOptions columnName1="name" columnName2="code" tableName="lineclassdic" valueName="linetypeColl"/>
      <html:select property="lineType" styleClass="selecttext" style="width:160">
		<html:option value="">不限</html:option>
    ...
    <template:formSubmit>
      <td>
        <html:submit styleClass="button">查 询</html:submit>
      </td>
      <td>
        <html:reset styleClass="button">取 消</html:reset>
      </td>
       <td>
        <html:button  property="button" styleClass="button" onclick="toGetBack()">返 回</html:button>
      </td>
    </template:formSubmit>
  </template:formTable>
</html:form>
