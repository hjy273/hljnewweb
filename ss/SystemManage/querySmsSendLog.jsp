...
function autoGetEndDate(){
  if(document.forms[0].begindate.value!=""){
    ...
    d2.setTime(d1.getTime()+7*24*3600*1000);
    value=d2.getYear()+"/";
...
function onChangePhone(){
  ...
  */
  ...
  else document.all.simid.value="";
}
</script>
<br>
<%
if(userInfo.getRegionID().substring( 2, 6 ).equals( "0000" ) && "".equals("1")){%>
<body onload="onChangeCon()">
<% }else{%>
<body>
<% }%>
<template:titile value="查询短信发送日志"/>
<html:form method="Post" action="/smsLogAction.do?method=querySMS_SendLog" >
  <template:formTable contentwidth="300" namewidth="280">
      <!--  <logic:notEmpty name="reginfo">
        ...
        <logic:notEmpty name="coninfo">
          <template:formTr name="代维单位" tagID="conTr" isOdd="false">
            <select name="contractorid" class="inputtext" style="width:180px" id="cId" >
    ...
    <template:formTr name="接收设备手机号码" isOdd="false" >
      <html:text property="simid" styleClass="inputtext" style="width:180;" maxlength="11"/>
    </template:formTr>
          <logic:present name="siminfo">
              <select name="phoneworkId"   style="display:none">
                <logic:iterate id="uinfoId" name="siminfo">
                    <option value="<bean:write name="uinfoId" property="ownerid"/>"><bean:write name="uinfoId" property="simnumber"/></option>
                </logic:iterate>
              </select>
          </logic:present>

    <template:formTr name="开始日期">
      <html:text property="begindate" styleClass="Wdate" style="width:180;" styleId="begindate"
       onfocus="WdatePicker({dateFmt:'yyyy/MM/dd HH:mm:ss',maxDate: '%y-%M-%d-%H-%m-%s'})"  readonly="true"/>
    </template:formTr>
    <template:formTr name="结束日期" isOdd="false">
      <html:text property="enddate" styleClass="Wdate" style="width:180;" 
        onfocus="WdatePicker({dateFmt:'yyyy/MM/dd HH:mm:ss',minDate:'#F{$dp.$D(\'begindate\')}',maxDate: '%y-%M-%d-%H-%m-%s'})" readonly="true"/>
    </template:formTr>
    <template:formSubmit>
      <td>
