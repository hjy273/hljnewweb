...
function doUpAccident(idValue, statusFlag){
    ...
    if(statusFlag == "处理中"){
     var url = "${ctx}/accidentAction.do?method=loadAccident&id=" + idValue +"&statusFlag=2" ;
      self.location.replace(url);
    }
    if(statusFlag == "已处理"){
      var url = "${ctx}/accidentAction.do?method=loadAccident&id=" + idValue +"&statusFlag=3";
      self.location.replace(url);
    }
    if(statusFlag == "已忽略"){
      alert("该障碍已经忽略！");
...
</script><template:titile value="障碍查询列表"/>
