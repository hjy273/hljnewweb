...
function onChangeContractor(){
  ...
  for( i=0;i<document.all.deptworkId.options.length;i++){
    if(document.all.deptworkId.options[i].text.substring(0,document.all.deptworkId.options[i].text.indexOf("-"))== document.all.rId.value){
      ...
      document.all.cId.options[k-1].value=document.all.deptworkId.options[i].value;
      document.all.cId.options[k-1].text=document.all.deptworkId.options[i].text.substring(document.all.deptworkId.options[i].text.lastIndexOf("-")+1,document.all.deptworkId.options[i].text.length);
    }
  }
  if(k==0)
...
<template:titile value="查询所有审批单信息"/>
<html:form onsubmit="return isValidForm()" method="Post" action="/PartRequisitionAction.do?method=showAllAudit">
  ...
  <template:formTable contentwidth="300" namewidth="200">
    ...
    <logic:notEmpty name="deptlist">
      <template:formTr name="移动公司" isOdd="false">
        <select name="deptid" class="inputtext" style="width:180px" id="cId"  onchange="onChangeMinterial();" >
