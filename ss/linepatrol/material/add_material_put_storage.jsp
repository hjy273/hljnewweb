  ...
  function onselectchange(){
   ...
   //脚本生成的删除按  钮的删除动作
	...
	<body>
		...
		<template:titile value="填写材料入库单" />
		<html:form action="/material_put_storage.do?method=addMaterialPutStorage"
			styleId="addForm">
				...
				<table width="90%" border="0" class="tabout" cellpadding="0"
					...
					<tr class=trcolor>
						...
						<td colspan="3" class="tdl">
							<html:hidden property="type" name="bean"/>
							<bean:define id="applyType" name="bean" property="type"
									...
									<tr id="rows<%=i%>">
										...
										<td>
											...
											<input type="hidden" id="text_applyrows<%=i%>"
												value="<%=count[i]%>" name="applyCount" size="6">
										</td>
										<td>
			...
			<p align="center">
				...
				<html:button property="action" styleClass="button" styleId="savesub"
					onclick="saveForm(this)">保存入库单</html:button>
				<html:button property="action" styleClass="button" styleId="addsub"
					onclick="checkForm(this)">提交入库单</html:button>
