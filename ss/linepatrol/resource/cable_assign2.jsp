...
<script language="javascript">
	...
	function search() {
        ...
        var url="${ctx}/resAction.do?method=searchCable&segmentname="+segmentname+"&maintenanceId="+maintenanceId+"&subline="+subline;
	...
	function goAssign1(){
		var maintenanceId = document.getElementById("maintenanceId").value;
		var url="${ctx}/resAction.do?method=cableAssignOne&maintenanceId="+maintenanceId;
...
<body onload="search()">
	<html:form method="Post"
		action="/resAction.do?method=cableAssignThree">
		<table border="0" align="center" cellspacing="0" cellpadding="0">
			<tr>
				<td valign="top">
					<input type="hidden" name="maintenanceId" id="maintenanceId" value="${maintenanceId }">
					光缆名称：<input type="text" name="segmentname" id="segmentname">
					...
					<select name="target" style="width: 200px" size="15"
						...
						<c:if test="${not empty list}">
							<c:forEach items="${list}" var="bean">
								<option value="<bean:write name='bean' property='kid'/>"><bean:write name='bean' property="segmentname"/></option>
							</c:forEach>
						</c:if>
					</select>
				</td>
