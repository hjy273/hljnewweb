	...
	<body>
		...
		<template:formTable contentwidth="300" namewidth="300">
			<template:formTr name="工程名称" isOdd="false">
				<input type="hidden" name="id" value="${pipe.id}"/>
				<input type="text" class="inputtext" style="width:200px" value="${pipe.workName}" name="workName" id="workName"/>
				<font color="red">*</font>
			</template:formTr>
			<template:formTr name="区域" isOdd="true">
				<apptag:quickLoadList cssClass="" name="scetion" keyValue="${pipe.scetion}" listName="terminal_address" type="radio"/>
			</template:formTr>
			<template:formTr name="管道地点" isOdd="false">
				<input type="text" class="inputtext" style="width:200px" value="${pipe.pipeAddress}" name="pipeAddress" id="pipeAddress"/>
				<font color="red">*</font>
			</template:formTr>
			<template:formTr name="管道路由" isOdd="true">
				<input type="text" class="inputtext" style="width:200px" value="${pipe.pipeLine}" name="pipeLine" id="pipeLine"/>
				<font color="red">*</font>
			</template:formTr>
			<template:formTr name="管道规模" isOdd="false">
				沟 <input type="text" class="inputtext validate-number" style="width:80px" value="${pipe.pipeLengthChannel}" name="pipeLengthChannel" id="pipeLengthChannel"/>
				孔 <input type="text" class="inputtext validate-number" style="width:80px" value="${pipe.pipeLengthHole}" name="pipeLengthHole" id="pipeLengthHole"/>公里
				<font color="red">*</font>
			</template:formTr>
			<template:formTr name="移动规模" isOdd="true">
				沟 <input type="text" class="inputtext validate-number" style="width:80px" value="${pipe.mobileScareChannel}" name="mobileScareChannel" id="mobileScareChannel"/>
				孔 <input type="text" class="inputtext validate-number" style="width:80px" value="${pipe.mobileScareHole}" name="mobileScareHole" id="mobileScareHole"/>公里
				<font color="red">*</font>
			</template:formTr>
			...
			<template:formTr name="交维日期" isOdd="false">
				<input type="text" class="inputtext" style="width:200px" value="<bean:write name="pipe" property="finishTime" format="yyyy/MM/dd"/>" name="finishTime" id="finishTime" onfocus="WdatePicker({dateFmt:'yyyy/MM/dd'})"/>
				<font color="red">*</font>
			</template:formTr>
			...
			<template:formTr name="维护单位">
				...
				<html:select property="maintenanceId" name="pipe" styleClass="inputtext" style="width:200px">
			...
			<template:formTr name="竣工图纸数量" isOdd="false">
				<input type="text" class="inputtext" style="width:200px" value="${pipe.picture}" name="picture"/>
			</template:formTr>
			<template:formTr name="备注" isOdd="false">
				<input type="text" class="inputtext" style="width:200px" value="${pipe.remark}" name="remark"/>
			</template:formTr>
		</template:formTable>
		<template:formSubmit >
			<input type="hidden" name="operator" value="update" />
			<td><input type="submit" class="button" name="submit" value="保存"/></td>
			<td><input type="button" class="button" onclick="history.go(-1);" value="返回"/></td>
		</template:formSubmit>
		</html:form>
		<template:cssTable/>
	</body>
