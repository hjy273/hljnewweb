...
<html>
	...
	<body>
		...
		<html:form action="/dispatchExamAction.do?method=examDispatch" styleId="form1">
							...
							<logic:notEmpty name="bean" property="acceptUserList">
								<logic:iterate id="oneAcceptUser" name="bean"
									...
									<tr>
										...
										<td style="text-align: center;">
											<logic:equal value="sign_in_task" property="task_state"
												name="oneAcceptUser">
											签收中
										</logic:equal>
											<logic:equal value="tranfer_sign_in_task"
												property="task_state" name="oneAcceptUser">
											签收中
										</logic:equal>
											<logic:equal value="refuse_confirm_task"
												property="task_state" name="oneAcceptUser">
											拒签确认中
										</logic:equal>
											<logic:equal value="reply_task" property="task_state"
												name="oneAcceptUser">
											反馈中
										</logic:equal>
											<logic:equal value="check_task" property="task_state"
												name="oneAcceptUser">
											反馈审核中
										</logic:equal>
		...
		<div align="right" style="height: 30px; line-height: 30px; margin:0px auto; width: 90%;"><a onclick="showOrHiddeInfo();" style="cursor: pointer;">显示/隐藏</a></div>
			<script type="text/javascript">
				listSignInTask();
				listReplyTask();
			</script>
		...
		<c:if test="${examFlag=='unexam'}">
				<apptag:appraiseDailyDaily businessId="${replyId}"
					contractorId="${contractorId}" businessModule="sendtask"
					tableClass="" tableStyle="width:90%;border:0px;"></apptag:appraiseDailyDaily>
				<apptag:appraiseDailySpecial businessId="${replyId}"
					contractorId="${contractorId}" businessModule="sendtask"
					tableClass="" tableStyle="width:90%;border:0px;"></apptag:appraiseDailySpecial>
			</c:if>
			<c:if test="${examFlag=='examed'}">
				<apptag:appraiseDailyDaily businessId="${replyId}"
					contractorId="${contractorId}" businessModule="sendtask"
					tableClass="" tableStyle="width:90%;border:0px;" displayType="view"></apptag:appraiseDailyDaily>
				<apptag:appraiseDailySpecial businessId="${replyId}"
					contractorId="${contractorId}" businessModule="sendtask"
					tableClass="" tableStyle="width:90%;border:0px;" displayType="view"></apptag:appraiseDailySpecial>
			</c:if>
			<p align="center">
