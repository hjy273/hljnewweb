        ...
        <div class="TabbedPanelsContentGroup">
          <div class="TabbedPanelsContent">
         	<c:forEach var="cableLevel" items="${cableLevels}">
	         	...
	         	&nbsp;&nbsp;&nbsp;巡回时段:<input name="startRoute_${cableLevel.key}" class="text_line" readonly="readonly" style="width:35px;" value="${startTime}" type="text" />--<input name="endRoute_${cableLevel.key}" class="text_line" value="${endTime}" readonly="readonly" style="width:35px;" type="text" />
...
<div style="text-align: center; width:100%">
	<c:choose>
		<c:when test="${flag eq 'safeguard'}">
			<input name="button" type="button" value="关闭" class="button" onclick="closeWin();"/>
		</c:when>
		<c:otherwise>
			<input name="button" type="button" value="返回" class="button" onclick="history.back()"/>
		</c:otherwise>
	</c:choose>
</div>
<script type="text/javascript">
