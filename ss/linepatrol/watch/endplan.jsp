...
<body style="vertical-align:bottom;">
	<template:titile value="盯防计划终止" />
	<form id="add_form" name="add_form" method="post" action="${ctx}/watchAction.do?method=endWatchPlan" onsubmit="return check();">
	...
	<table width="80%" align="center" border="1" class="tabout" cellpadding="0" cellspacing="1">
	  ...
	  <tr>
	    <td class="tdulleft">终止方式：</td>
	    <td class="tdulright">
	    	<c:choose>
	    		<c:when test="${empty hiddangerEndplan}">
	    			<input type="radio" name="type" value="1" checked />隐患已消除
		    		<input type="radio" name="type" value="2" />重新制定计划
	    		</c:when>
	    		<c:otherwise>
	    			<input type="radio" name="type" value="1" <c:if test="${hiddangerEndplan.endType eq '1'}">checked</c:if> />隐患已消除
		    		<input type="radio" name="type" value="2" <c:if test="${hiddangerEndplan.endType eq '2'}">checked</c:if> />重新制定计划
	    		</c:otherwise>
	    	</c:choose>
	    </td>
	  </tr>
	  <tr>
	    <td class="tdulleft">终止原因：</td>
	    <td class="tdulright">
	    	<textarea name="reason" id="reason" rows="4" cols="45" ><c:if test="${!empty hiddangerEndplan}">${hiddangerEndplan.reason}</c:if></textarea><font color="red">*</font>
	    </td>
	  </tr>
	  <tr>
	  ...
	  <apptag:approverselect label="审核人" inputName="approver" 
	   approverType="approver" objectType="LP_HIDDANGER_MAKEPLAN" objectId="${hiddangerId}" 
	   spanId="approverSpan" inputType="radio" notAllowName="reader"/>
	  <apptag:approverselect label="抄送人" inputName="reader" 
	   approverType="reader" objectType="LP_HIDDANGER_MAKEPLAN" objectId="${hiddangerId}" 
	   spanId="readerSpan" notAllowName="approver"/>
	  <apptag:approve labelClass="tdulleft" displayType="view" objectId="${sp.id}" objectType="LP_HIDDANGER_ENDPLAN" />	
	<template:formSubmit>
		<td>
