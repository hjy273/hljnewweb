...
<script type="text/javascript">
	...
	function read(){
		var url = '${ctx}/hiddangerAction.do?method=read&id=${hiddangerId}&type=LP_HIDDANGER_ENDPLAN';
...
<body style="vertical-align:bottom;">
	<template:titile value="盯防计划终止审核" />
	<form id="add_form" name="add_form" method="post" action="${ctx}/watchAction.do?method=endPlanApprove">
	...
	<table width="80%" align="center" border="1" class="tabout" cellpadding="0" cellspacing="1">
	  ...
	  <tr>
	    ...
	    <td class="tdulright">
	    	<c:choose>
	    		<c:when test="${hiddangerEndplan.endType eq '1'}">
	    			隐患已消除
	    		</c:when>
	    		<c:otherwise>
	    			重新制定计划
	...
	<c:choose>
		<c:when test="${!readOnly}">
			<apptag:approve labelClass="tdulleft" areaStyle="width: 300px;" displayType="input" objectId="${hiddangerId}" objectType="LP_HIDDANGER_ENDPLAN" />
			<template:formSubmit>
				<td>
...
<script type="text/javascript">
	...
	jQuery("#add_form").bind("submit",function(){processBar(add_form);});
})
	</script>
</html>