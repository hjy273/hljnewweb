...
<template:titile value="特巡计划终止时间审核"/>
</c:if>
<c:if test="${operator=='transfer'}">
<template:titile value="特巡计划终止时间转审"/>
</c:if>
<form id="add_form" name="add_form" method="post" action="${ctx}/specialEndPlanAction.do?method=approveSpecialEndPlan">
...
<c:if test="${not empty specialEndPlan}">
	<table align="center" cellpadding="1" style="border-bottom:0px;border-top: 0px;" cellspacing="0" class="tabout" width="80%">
		...
		<tr class="trcolor">
			...
			<td class="tdulright">
				<input type="hidden" name="id" value="${specialEndPlan.id }"/>
				<input type="hidden" name="planId" value="${specialEndPlan.planId }"/>
				<input type="hidden" name="creater" value="${specialEndPlan.creater }"/>
				<input type="hidden" name="operator" value="${operator }"/>
				<input type="hidden" name="endType" value="${specialEndPlan.endType }"/>
