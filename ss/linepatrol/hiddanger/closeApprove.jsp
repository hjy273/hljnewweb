...
<head>
	...
	<script type="text/javascript">
		...
		function read(){
			var url = '${ctx}/hiddangerAction.do?method=read&id=${registBean.id}&type=LP_HIDDANGER_CLOSE';
...
<body onload="changeStyle()">
	<template:titile value="隐患关闭审核" />
	<html:form action="/hiddangerAction.do?method=closeApprove" styleId="form">
		...
		<template:formTable namewidth="200" contentwidth="500">
			<input type="hidden" name="hiddangerId" value="${registBean.id}">
			...
			<c:choose>
				<c:when test="${!registBean.readOnly}">
					<apptag:approve labelClass="tdulleft" areaStyle="width: 300px;" displayType="input" objectId="${registBean.id}" objectType="LP_HIDDANGER_CLOSE" />
					<template:formSubmit>
						<td>
