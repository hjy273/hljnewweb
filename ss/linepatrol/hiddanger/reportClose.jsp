...
<body onload="changeStyle()">
	...
	<html:form action="/hiddangerCloseAction.do?method=reportClose" styleId="form" onsubmit="return check();">
		...
		<template:formTable namewidth="200" contentwidth="500">
			...
			<template:formTr name="隐患解除原因">
				<html:textarea property="reliefReason" styleId="reliefReason" styleClass="inputtextarea required" rows="4" cols="45" /><font color="red">*</font>
			</template:formTr>
			<template:formTr name="备注">
				<html:textarea property="remark" styleId="remark" styleClass="inputtextarea" rows="4" cols="45" />
			</template:formTr>
			<template:formTr name="盯防计划">
				<jsp:include page="statIframe.jsp" />
			</template:formTr>
			<apptag:approverselect label="审核人" inputName="approver"
			 approverType="approver" objectType="LP_HIDDANGER_MAKEPLAN" objectId="${registBean.id}" 
			 spanId="approverSpan" inputType="radio" notAllowName="reader" />
			<apptag:approverselect label="抄送人" inputName="reader"
			 approverType="reader" objectType="LP_HIDDANGER_MAKEPLAN" objectId="${registBean.id}"
			 spanId="readerSpan" notAllowName="approver"/>
			<apptag:approve labelClass="tdulleft" displayType="view" objectId="${registBean.id}" objectType="LP_HIDDANGER_CLOSE" />
			<template:formSubmit>
				<td>
