...
<html>
	...
	toViewForm = function(id) {
		window.location.href = "${ctx}/appraiseTableYearAction.do?method=viewTable&id="+id;
	}
	function toDelete(id){
		if(confirm("确定要删除该模板？")){
			location="${ctx}/appraiseTableYearAction.do?method=deleteTable&id="+id;
		}
	}
</script>
			...
			<display:table name="sessionScope.tables" id="table" pagesize="18" style="width:99%" >
				<logic:notEmpty name="table">
					...
					<display:column media="html" title="操作">
						...
						| <html:link action="appraiseTableYearAction.do?method=exportTable&id=${id}">导出</html:link>
					</display:column>
				</logic:notEmpty>
			</display:table>
