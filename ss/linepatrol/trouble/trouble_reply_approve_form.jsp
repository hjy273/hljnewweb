	...
	<link type="text/css" rel="stylesheet"
		...
		<script type="text/javascript">
			...
			toApproveForm=function(idValue,act){
				if(act=="1"){
					window.location.href = "${ctx}/replyApproveAction.do?method=addApproveReplyForm&replyid="+idValue+"&act=approve";
				}
				if(act=="2"){
					window.location.href = "${ctx}/replyApproveAction.do?method=addApproveReplyForm&replyid="+idValue+"&act=transfer";
				}				
			}
			
			function closetrouble(){
				if(confirm("该故障所有反馈为协办!是否确定要关闭故障!")){
					var troubleid = document.getElementById("troubleid").value;
					window.location.href = "${ctx}/replyApproveAction.do?method=closeTroubleInfo&troubleid="+troubleid;
				}
			}
			
			function toCloseReply(){
				if(confirm("主办反馈单已经审核!是否确定要关闭所有协办反馈单!")){
					var troubleid = document.getElementById("troubleid").value;
					window.location.href = "${ctx}/replyApproveAction.do?method=closeTroubleInfo&troubleid="+troubleid+"&act=closereply";
				}
			}
				 		...
				 		<c:if test="${varloop.index%2!=0}">
				 		  <tr class=trwhite>
							...
							<bean:define id="rid" property="id" name="reply"></bean:define>
								   <logic:equal value="0" property="state" name="reply">
									  <logic:equal value="false" property="isread" name="reply">
									    <a href="javascript:toApproveForm('<%=rid%>','1')">审核</a>|
										<a href="javascript:toApproveForm('<%=rid%>','2')">转审</a>|
									  </logic:equal>
								  </logic:equal>
