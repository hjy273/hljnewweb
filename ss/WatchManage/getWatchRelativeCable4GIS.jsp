...
function presetData(fID){
  ...
  if(fID == "1"){
    ...
    try{
     // alert(opener.watchBean.involvedsegmentlist.length);
      for(var i = 0; i < opener.watchBean.involvedsegmentlist.length; i++ ){
        txt = opener.watchBean.cablename[i].value;
        val = opener.watchBean.involvedsegmentlist[i].value;
        //alert(txt+" "+val);
        toList.options[toList.length] = new Option(txt,val);
      }
      for(var i = 0; i < opener.watchBean.involvedsegmentlist.length; i++ ){
        for(var k = 0; k < sublineBean.sublinecablelist.options.length; k++){
          if(opener.watchBean.involvedsegmentlist[i].value == sublineBean.sublinecablelist.options[k].value){
      ...
      if(toList.length == 0){
        txt = opener.watchBean.cablename.value;
        val = opener.watchBean.involvedsegmentlist.value;
        toList.options[toList.length] = new Option(txt,val);
        for(var k = 0; k < sublineBean.sublinecablelist.options.length; k++){
          if(opener.watchBean.involvedsegmentlist.value == sublineBean.sublinecablelist.options[k].value){
            sublineBean.sublinecablelist.options[k] = null;
            continue;
          }
        }
...
function setParent(){
  ...
  var conStr = "<br><table>";
  for(var i = 0; i < toList.options.length; i++){
    conStr += "<tr><td><input type=hidden name=involvedsegmentlist value="+toList.options[i].value+">";
...
<table width="95%" border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
    <td height="24" align="center" class="title2">添加修改受影响光缆信息</td>
    ...
    <tr>
      <td width="45%" align="center">
        ...
        <table width="90%"  border="0" cellpadding="0" cellspacing="0"  class="tabout">
          <tr>
            <th width="160" class="thlist">受影响的光缆</th>
          </tr>
          <tr>
            <td align="center">
