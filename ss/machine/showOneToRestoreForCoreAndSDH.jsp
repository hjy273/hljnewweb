...
<html>
	<head>
		...
		<script type="text/javascript">
			function restore(pid,tid,type) {
				var url = "PollingContentAction.do?method=restoreAEqu&pid="+pid+"&tid="+tid+"&type="+type;
	...
	<body>
		...
		<%
			...
			String state = null;
		 %>
		 <template:titile value="机房巡检设备回复一览表"/>
		 
		<display:table name="sessionScope.oneTaskList" id="currentRowObject" pagesize="1">
			<%
			...
			<display:column media="html" class="subject" headerClass="subject" title="操作">
				<a href="javascript:restore('<%=pid %>','<bean:write name="tid"/>','<bean:write name="type"/>')">添加回复内容</a>
			</display:column>
		</display:table>
