...
<html>
	<head>
		...
		<script type="text/javascript">
			...
			function signTask(id,type) {
				var url="PollingTaskAction.do?method=showOneForSign&id="+id+"&type="+type;
	...
	<body>
		<br>
		<template:titile value="待签收机房巡检任务一览表"/>
		<%
			...
			String title = null;
		 %>
		<display:table name="sessionScope.taskList" requestURI="${ctx}/MobileTaskAction.do?method=getTaskForCon" id="currentRowObject" pagesize="18">
			<%
			...
			<display:column media="html" title="操作" class="subject" headerClass="subject">
				<a href="javascript:signTask('<%=id %>','<%=type %>')">签收</a>
			</display:column>
