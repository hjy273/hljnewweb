...
//取消添加
function Cancel(){
	location.href = "${ctx}/planManage/addYearPlan.jsp";
}
function save(){
	if($('approverid').value == ''){
		alert("目标处理人不能为空！");
			   ...
			   <table name="tasklisttable" border="0" cellspacing="0" cellpadding="5">
			   	<tr>
			   		...
			   		<logic:iterate id="tasklist" name="taskList" indexId="index">
			   			<input name=linelevel type="text" class="inputtext" style="border:0;background-color:transparent;width:60px;cursor:hand" value="<bean:write name="tasklist" property="lineleveltext"/>" idValue="<bean:write name="tasklist" property="id"/>"  readonly onclick="//loadTask(this)" />
			   			<input name=taskname type="text" class="inputtext" style="border:0;background-color:transparent;width:120px;cursor:hand" value="<bean:write name="tasklist" property="describtion"/>" idValue="<bean:write name="tasklist" property="id"/>"  readonly onclick="//loadTask(this)" />
			   			...
			   			<input name="edit" onclick="editTask(<%=index %>)" type="button" class="button" value="修改" /><input name="删除" onclick="removeTask(<%=index %>)" type="button" class="button" value="删除" /><br/>
			   		</logic:iterate>
			   		</td>
			   	</tr>
			   </table>
