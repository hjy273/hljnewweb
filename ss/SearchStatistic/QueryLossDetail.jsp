...
function subClick(){
	...
	if(rtype == '3'){
  	...
  	if(fdate ==2){
        var y = queryConditionForm.year.value;
        var m = queryConditionForm.month.value;
    	queryConditionForm.begindate.value = y + "/" + m + "/01";
        //alert(queryConditionForm.begindate.value);
       	var d = 30;
	       if(m == 2){//是2月
	  ...
	  queryConditionForm.enddate.value = y + "/" + m + "/" + d;
      //alert(queryConditionForm.enddate.value);
    }
...
function getDep(){
	...
	setDisable(queryConditionForm.patrolid, true);

	var deptid = queryConditionForm.deptid.value;
	var URL = "${ctx}/SearchStatistic/getSelect.jsp?deptid="+deptid;
...
function showLine(){
        ...
        for( i=iIndex;i<iArray.length;i++){
          	...
          	document.all.lID.options[0].text="不限";
...
function conShow(){
    ...
    function onChangeExe(){
	  ...
	  if(document.all.wID.value==""){
	        ...
	        document.all.pID.options[0].text="不限";
	        return ;
    ...
    function onchangesubline(){
	  ...
	  if(document.all.lID.value==""){
	    	document.all.slId.options.length=1;
	    	document.all.slId.options[0].value='';
	        document.all.slId.options[0].text="不限";
	        return ;
	  }
      var iArray = document.all.sublId.options;   //所隐藏的数组
      var iIndex = 0;
      var iStart = 0; //
      var iEnd = iArray.length - 1;
      var iCount = 0;
      var iSeek = document.all.lID.value;       // 所要查找的值,上级菜单的值
      while(iSeek != ""){
           iCount++;
         iIndex =parseInt( ( iStart + iEnd )/2);
         if( iArray[iIndex].text.substring(8,16) < iSeek ){ //要替换成下级菜单的比较条件
                iStart = iIndex;
         }else if( iArray[iIndex].text.substring(8,16) > iSeek ){//要替换成下级菜单的比较条件
                    iEnd = iIndex;
         }else{
             if(iIndex == 0){
                break;
            }else{
                if(iArray[iIndex-1].text.substring(8,16) == iSeek){ //要替换成下级菜单的比较条件
                       iEnd = iIndex;
                }else{
                    break;
                }
        ...
        if(iIndex==iEnd-1 && iArray[iEnd].text.substring(8,16) != iSeek){ //要替换成下级菜单的比较条件
            index = iArray.length;
            document.all.slId.options.length=0;
            break;
        }
      }
      ...
      for(var i=iIndex;i<iArray.length && iArray[i].text.substring(8,16)==document.all.lID.value;i++){
          k++;
          document.all.slId.options.length=k;   //document.all.slId.options 要设置的select

          document.all.slId.options[0].value="";
          document.all.slId.options[0].text="不限";

          document.all.slId.options[k-1].value=document.all.sublId.options[i].value;
          document.all.slId.options[k-1].text=document.all.sublId.options[i].text.substring(18,document.all.sublId.options[i].text.length);
      }
    }
...
<template:titile value="计划漏检明细查询"/>
<html:form method="Post" action="/StatisticAction.do?method=getLossDetail">
<template:formTable namewidth="200" contentwidth="300">
			...
			<template:formTr name="巡检线路" tagID="lineTr" style="display:none">
				<select name="lineid" class="selecttext" style="width:225" id="lID" onChange="onchangesubline()">
					<option>
					</option>
				</select>
			...
			<template:formTr name="巡检线段" tagID="sublineTr" style="display:none">
				<span id="slineSpan">
	         	<select name="sublineid"  id="slId" Class="inputtext" style="width:225" >
	         	</select>
				</span>
				
    		</template:formTr>
    		<logic:present name="lsubline">
				<select name="sublId" style="display:none" id="sublId">
					<logic:iterate id="lslId" name="lsubline">
						<option value="<bean:write name="lslId" property="sublineid"/>">
							<bean:write name="lslId" property="regionid" />--<bean:write name="lslId" property="lineid" />--<bean:write name="lslId" property="sublinename" />
						</option>
					</logic:iterate>
				</select>
				</logic:present>	
...
<template:formSubmit>
    <td width="85">
        <html:button property="action" styleClass="button" onclick="subClick()" >查询</html:button>
    </td>
    <td width="85">
