...
<html>
	...
	<body>
		<br>
		<template:titile value="查询中继站计划" />
		<form method="post"
			action="<%=request.getContextPath()%>/station_plan.do?method=list"
			id="queryForm">
			<template:formTable namewidth="200" contentwidth="200">
				<html:hidden property="reset_query" value="1" />
				<template:formTr name="所属地州">
					<select name="region_id" style="width:250;" class="inputtext"
				...
				<template:formTr name="计划类型">
					<select name="plan_type" style="width:250;" class="inputtext">
						<option value="">
							不限
						</option>
						<option value="01">
		...
		loadStation(queryForm.region_id.value);
		</script>
	</body>
</html>
