...
<%
  ...
  String daysForPlanStart = gisip.getDaysfornoticeplanstart() + "天后即将开始执行的计划列表";
%>
       ...
       var x=document.getElementsByName("choice_id").length; 
          ...
          if(x==1){
            if(ec.choice_id.checked==true){
                ...
                if(confirm('确定发送短信吗？')){
		            ec.action="${ctx}/SMNoticePlanStartAction.do?method=sendMessage&strsimnumberlist=" + thevalue;
		            ec.submit();
			    }
            }else{
                alert("请选择消息发送对象");           
...
<body style="margin:10px;">
	...
	var="mytable"
	action="${pageContext.request.contextPath}/SMNoticePlanStartAction.do?method=showNoticeList"
	...
	<ec:row highlightRow="true">
	  ...
	  sortable="false"
	  >
	  <input name="choice_id" type="checkbox"  value="${mytable.simnumber},${mytable.begindate}">
	</ec:column>
